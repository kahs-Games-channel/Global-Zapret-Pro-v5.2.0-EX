@echo off
set "LOCAL_VERSION=Global-Zapret-Pro-v5.2.0 EX"
...
tasklist /FI "IMAGENAME eq winws_zapret.exe" | find /I "winws_zapret.exe" > nul
if !errorlevel!==0 (
    call :PrintGreen "Bypass (winws_zapret.exe) is RUNNING."
) else (
    call :PrintRed "Bypass (winws_zapret.exe) is NOT running."
)
...
tasklist /FI "IMAGENAME eq winws_zapret.exe" | find /I "winws_zapret.exe" > nul
if !errorlevel!==0 (
    taskkill /IM winws_zapret.exe /F > nul
)
...
for /f "tokens=*" %%a in ('type "!selectedFile!"') do (
    ...
    echo !line! | findstr /i "%BIN%winws_zapret.exe" >nul
    if not errorlevel 1 (
        set "capture=1"
    )
    ...
    if !capture!==1 (
        if not defined args (
            set "line=!line:*%BIN%winws_zapret.exe"=!"
        )
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%winws_zapret.exe\" !ARGS!" DisplayName= "zapret" start= auto
...
if !winws_running! neq 0 if !windivert_running!==0 (
    call :PrintYellow "[?] winws_zapret.exe is not running but WinDivert service is active. Attempting to delete WinDivert..."
